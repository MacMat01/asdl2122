...
public abstract class Graph<L> {
     ...
     *                                      non esiste nel grafo
     */
    public abstract boolean addWeightedEdge(GraphNode<L> node1, GraphNode<L> node2, double weight);
    ...
    public int getDegreeOf(GraphNode<L> node) {
        if (node == null) throw new NullPointerException("Tentativo di calcolare il grado di un nodo null");
        if (this.getNode(node) == null)
            throw new IllegalArgumentException("Tentativo di calcolare il grado di un nodo non " + "presente in questo grafo");
        if (!this.isDirected()) return this.getEdgesOf(node).size();
        else return this.getEdgesOf(node).size() + this.getIngoingEdgesOf(node).size();
    }
    ...
    public int getDegreeOf(L label) {
        if (label == null) throw new NullPointerException("Tentativo di calcolare il grado di un nodo null");
        if (this.getNode(label) == null)
            throw new IllegalArgumentException("Tentativo di calcolare il grado di un nodo non " + "presente in questo grafo");
        if (!this.isDirected()) return this.getEdgesOf(label).size();
        else return this.getEdgesOf(label).size() + this.getIngoingEdgesOf(label).size();
    }
    ...
    public int getDegreeOf(int i) {
        ...
        try {
            ...
            // metodo
            throw new IndexOutOfBoundsException("Tentativo di calcolare il grado di un nodo con indice " + "non valido");
        }
